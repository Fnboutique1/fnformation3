...
// Répartit les 12 thèmes des 4 modules sur 16 semaines : chaque semaine
// comprend 1 à 2 courtes classes vidéo (15-25 min) + un devoir à rendre.
function buildWeeks(curriculum: { title: string; lessons: string[] }[]): WeekPlan[] {
  ...
  for (let i = 0; i < 16; i++) {
    ...
    const classes =
      i % 4 === 3
        ? [`Classe — Révision & pratique orale : "${topic}"`]
        : [`Classe 1 — ${topic} (théorie)`, `Classe 2 — ${topic} (mise en pratique)`];
    weeks.push({
      week: i + 1,
      classes,
